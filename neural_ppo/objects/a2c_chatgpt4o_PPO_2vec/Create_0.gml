...
total_reward = 0;
timestep = 0;
max_timesteps = 40; // Количество шагов до обновления
...
function getState() {
	return [x / room_width, y / room_height]; //[obj_target.x, obj_target.y,x / room_width, y / room_height];
}
...
policy_network = new mlp_array([2,3,3,4],[Tanh,Tanh,Tanh]);//policy_network = new mlp_array([2,3,4],[Tanh,Tanh]);

// Создаём сеть критика (Value network)
value_network = new mlp_array([2,4,4,1],[Tanh,Tanh,Tanh]);
